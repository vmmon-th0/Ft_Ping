...
fill_icmp_packet_v4 (struct ping_packet_v4 *ping_pkt)
{
    ...
    ping_pkt->hdr.code = 0;
    /* Using the mask ensures that the ID does not exceed 16 bits, which is a convention for ICMP packets */
    ...
    /* Remember to set the checksum to 0 since it will be calculated on the entire ICMP packet. */
